...
int main(int argc, char *argv[])
{
    ...
    if (!leftLegDevice.view(leftLegIVelocityControl2) ) { // connecting our device with "Velocity control 2" interface (configuring our device: speed, acceleration... and sending joint positions)
    ...
    if (!rightLegDevice.view(rightLegIVelocityControl2) ) { // connecting our device with "Velocity control 2" interface (configuring our device: speed, acceleration... and sending joint positions)
    ...
    if (!trunkDevice.view(trunkIVelocityControl2) ) { // connecting our device with "Velocity control 2" interface (configuring our device: speed, acceleration... and sending joint positions)
        ...
        return false;
            ...
            //-- SET CONTROL MODE
    int leftLegAxes;
    leftLegIVelocityControl2->getAxes(&leftLegAxes);
    std::vector<int> leftLegControlModes(leftLegAxes,VOCAB_CM_VELOCITY);
    ...
    int rightLegAxes;
    rightLegIVelocityControl2->getAxes(&rightLegAxes);
    std::vector<int> rightLegControlModes(rightLegAxes,VOCAB_CM_VELOCITY);
    ...
    int trunkAxes;
    trunkIVelocityControl2->getAxes(&trunkAxes);
    std::vector<int> trunkControlModes(trunkAxes,VOCAB_CM_VELOCITY);
    ...
    myRateThread.set("sagittal", &pidcontroller_ankle, &pidcontroller_trunk);
    //myRateThread.set("frontal", &readPort, velRightLeg, velLeftLeg, velTrunk, posTrunk, encTrunk,&pidcontroller_ankle_f, &pidcontroller_hip);
    myRateThread.start();
