...
public:
    ...
    void zmpref_generator(){
        ...
        if (_num<500){
            ...
            _num = 0;
            ref = ref + 0.005; // ZMP ref escalonada en intervalos de 5mm - unidades ref[m]
        }
...
*/
        ...
        // transformacion de las variables ZMP_FT a milimetros
        _xzmp00 = _xzmp0*1000; // xzmp01 in [mm]
        _yzmp00 = _yzmp0*1000; // yzmp01 in [mm]

        _xzmp11 = _xzmp1*1000; // xzmp11 in [mm]
        _yzmp11 = _yzmp1*1000; // yzmp11 in [mm]

        _xzmp01 = _xzmp*1000; // xzmp2 in [mm]
        _yzmp01 = _yzmp*1000; // yzmp2 in [mm]

        X2 = X*1000;  // unidades X2[mm]
...
//        velLeftLeg->velocityMove(4, -vel);
    ...
    void printData()
    {
        ...
        cout << endl << "La ref es = ("<< ref << ") [m]" << endl;
    ...
    void saveInFileCsv()
    {
        if(n==1){
            ...
            fprintf(fp,",ZMPref_mm,eval_x.zmp_ref_mm,evalx.y_mm");
        }
        ...
        fprintf(fp,",%.2f", _xzmp00);
        fprintf(fp,",%.2f", _yzmp00);
...
private:
    ...
    float _xzmp00, _yzmp00, _xzmp11, _yzmp11, _xzmp01, _yzmp01; // variables para graficar en milimetros
    float _xzmp; // Global x_ZMP
    float _yzmp; // Global y_ZMP
    float X, ref, angle_x, X2, zmp_ref, ref2, y2;
