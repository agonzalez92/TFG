...
Para poder llevar a cabo las pruebas el primer paso a realizar es fijar las variables del sistema como son, la masa del robot TEO, 62,6 kg, y la altura del modelo simplificado del mismo, es decir, la altura del péndulo invertido, medida desde el suelo al CoM, 0,893 m.

El siguiente paso fue realizar las pruebas sometiendo al robot a una serie de variaciones en el ángulo de los tobillos para simular perturbaciones pequeñas, con la excepción de que no volvía al ZMP inicial, para así poder modelar mejor la diferencia entre el ZMP esperado y el medido, estudiándose también su variación dinámica a lo largo del proceso. 
...
\begin{figure}[H]
\centering
\subfigure[]{\includegraphics[scale=0.5]{imagenes/apartado_5/55_1_altura_no_ajustada}}
\quad
\subfigure[]{\includegraphics[scale=0.4]{imagenes/apartado_5/55_2_altura_ajustada.pdf}}
...
Para la respuesta en régimen permanente de los experimentos se ha desarrollado la tabla \ref{tabla52}, en la que se muestra una comparación de la ubicación del ZMP deseado y medido entre el modelo LIPM y el modelo DLIPM. Como se puede observar se ha reducido el error estático en todos los puntos de trabajo. Ésta mejora puede observarse de manera gráfica en la figura %%poner la figura 20 del paper

En cuanto al régimen transitorio, la respuesta se muestra en la figura %%aquí la figura 21 del paper
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{imagenes/apartado_5/511_evolucion_zmp_dlipm.pdf}
...
\subsubsection{Respuesta del modelo cart-table a partir del modelo DLIPM}

En %%poner el tfm de juanlo 
el modelo cart-table presentaba errores en régimen permanente, como se puede observar en %%aqui añadir la figura del cart-table sin corregir.

Se decidió que, a partir del ZMP calculado en el modelo DLIPM se computara la altura del modelo cart-table, como se indica en la ecuación \ref{ec435}, y a partir de dicha altura, calcular el ZMP del modelo cart-table.

La corrección del error en régimen permanente puede observarse en la figura %%aquí poner la figura de los experimentos de la carpeta imu2 dentro de la carpeta de juanmi.
en la que se indica de color negro la referencia a la que debían llegar los ZMP tanto del modelo DLIPM (en rojo) como el modelo cart-table (en verde).

\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{imagenes/apartado_5/test2_imu}
