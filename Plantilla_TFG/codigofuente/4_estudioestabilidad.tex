...
\caption{Plano Anatómico Sagital}
\label{figura41}
\end{figure}
...
Pero esta ecuación sólo valdría cuando el robot está apoyado sobre una pierna (simple apoyo). Sin embargo, el proyecto se ha desarollado estándo apoyadas las dos piernas (doble apoyo), por lo que la ecuación para el ZMP global es:
...
donde $x(t)$ indica el movimiento del CoM, $m$ es la masa del péndulo localizada en el CoM, $l$ la longitud del péndulo, $k_a$ la constante del muelle y $B_a$ la constante del amortiguador. El desplazamiento del CoM es tán pequeño que se puede asumir que $\sin\theta = \theta$, quedando la ecuación 4.24 como:
...
Por otra parte, el torque tmbién se puede obtener de la medición del ZMP como:
...
Las ecs del modelo cart-table para controlar el ZMP se obtienen sustituyendo \ref{ec419} y \ref{ec420} en \ref{ec415} y \ref{ec416} del modelo 3DLIPM,
...
que da el cáculo del ZMP en el borde del polígono de soporte (pie de la mesa). 
...
Para el modelo cart-table se necesitan aceleraciones en la dirección en la que se desplaza, por lo que cuando el robot está en posición estática y erguido en doble apoyo no existe dicha aceleración. Por ello sólo se pueden considerar modelos equivalentes cuando el robot realiza el paso, ya que al moverse el robot se crean aceleraciones en la misma dirección de desplazamiento. Esto queda reflejado en la siguiente ecuación: 

\begin{equation}
X_{ZMP_{FT_t}}\neq X_{ZMP_{IMU_t}}\Rightarrow X_{ZMP_{FT_{(t+1)}}}=X_{ZMP_{IMU_{(t+1)}}}
\label{ec428}
\end{equation}

Esta equivalencia se expondrá con más detalle en el apartado \ref{respuestaIMU}.
